...
def main(userCount,itemCount,testSet,trainVector,trainMaskVector,topN,epochCount,alpha,):
    ...
    for epoch in range(epochCount):
        ...
        for step in range(G_step):
            ...
            realData_zp = Variable(torch.ones_like(realData)).to(device) * eu+ Variable(torch.zeros_like(realData)).to(device) * ku_zp
        ...
        for step in range(D_step):
            ...
            d_loss = -np.mean(np.log(realData_result.detach().cpu().numpy()+10e-5) + np.log(1. - fakeData_result.detach().cpu().numpy()+10e-5)) + 0*regularization(fakeData_ZP,realData_zp)
        ...
        if( epoch%1==0):
            ...
            for testUser in testSet.keys():
                ...
                result = G(data.reshape(1,itemCount)) + Variable(copy.deepcopy(trainMaskVector[index])).to(device).to(device)
            ...
            print('Epoch[{}/{}],d_loss:{:.6f},g_loss:{:.6f},precision:{},precision:{},ndcg5:{},ndcg10:{}'.format(epoch, epochCount,
            d_loss.item(),
            g_loss.item(),precisions_5,precisions_10,ndcg_5,ndcg_10))
...
if __name__ == '__main__':
    ...
    result_precision, result_last = main(userCount, itemCount, testSet,trainVector, trainMaskVector, topN, epochs, alpha)
