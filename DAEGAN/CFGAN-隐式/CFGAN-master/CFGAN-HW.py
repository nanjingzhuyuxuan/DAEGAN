...
def main(userCount, itemCount, testSet, trainVector, trainMaskVector, topN, epochCount, alpha, wake_negativeList,
    ...
    for epoch in range(epochCount):
        ...
        for step in range(G_step):
            ...
            ku_zp = Variable(torch.tensor(n_items_pm )).to(device) + Variable(torch.tensor(eu_f)).to(device)
        ...
        for step in range(D_step):
            ...
            fakeData_ZP = fakeData * (eu_f + ku + eu)
        ...
        if (epoch % 1 == 0):
            ...
            print('Epoch[{}/{}],d_loss:{:.6f},g_loss:{:.6f},precision:{},precision:{},ndcg5:{},ndcg10:{}'.format(epoch,epochCount,d_loss.item(),g_loss.item(),
                                                                                                                 precisions_5,
                                                                                                                 precisions_10,
                                                                                                                 ndcg_5,
                                                                                                                 ndcg_10))
...
if __name__ == '__main__':
    ...
    trainVector, trainMaskVector, batchCount, wake_negativeList, trainVector_eu = data_fill.to_Vectors(trainSet, fillSet,userCount,itemCount, userList_test,)
